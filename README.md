...
**🚀 Live App:** [customerchurnprediction-pbdyvv9mszdvauultgp6lu.streamlit.app](https://customerchurnprediction-pbdyvv9mszdvauultgp6lu.streamlit.app/)
...
1. **Data preprocessing** — categorical features (`Geography`, `Gender`) are encoded using a saved `OneHotEncoder` and `LabelEncoder`, while numerical features are scaled with a saved `StandardScaler`.
2. **Model** — a trained Artificial Neural Network (Keras/TensorFlow, saved as `model.h5`) takes the processed features and outputs a churn probability between 0 and 1.
3. **Interface** — Streamlit collects user input, transforms it using the saved encoders/scaler, feeds it to the model, and displays the predicted probability along with a human-readable message.
...
```bash
git clone https://github.com/Rohit8022/Customer_Churn_Prediction.git
...
**Rohit Prajapati**
- GitHub: [@Rohit8022](https://github.com/Rohit8022)
